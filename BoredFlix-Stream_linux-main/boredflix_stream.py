...
class ItemConfirm(OneLineAvatarIconListItem):
    ...
    def set_icon(self, instance_check):
        ...
        def thrs(magnet_link: str, download: bool):
            global th1
            th1=multiprocessing.Process(target=webtorrent_stream,args=(magnet_link,download))
            th1.start()
        @mainthread
        def reset(*a):
            self.mod.dismiss()
        global webtorrent_stream
        def webtorrent_stream(magnet_link: str, download: bool):
            Clock.schedule_once(reset,3)
            subprocess.call('curl -X POST http://localhost:8080/shutdown',shell=True)
            try:
